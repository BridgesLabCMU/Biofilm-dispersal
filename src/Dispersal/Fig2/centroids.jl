...
function main()
    ...
    plot_filename = "WT_centroids"
    data_all = []
    in_out_all = []
    out_in_all = []
    random_all = []
    for i in 1:5
        data = readdlm(plots_folder*"/WT_replicate"*string(i)*"_processed_data_intensity.csv", ',')[1:end,1:end,1]
        in_out = readdlm(plots_folder*"/WT_replicate"*string(i)*"_processed_in_out_net_downsampled.csv", ',')[1:end,1:end,1]
        out_in = readdlm(plots_folder*"/WT_replicate"*string(i)*"_processed_out_in_net_downsampled.csv", ',')[1:end,1:end,1]
        random = readdlm(plots_folder*"/WT_replicate"*string(i)*"_processed_random_net_downsampled.csv", ',')[1:end,1:end,1]
    ...
    plot_filename = "WT_centroids_boxplot"
    in_out_integrals = []
    out_in_integrals = []
    random_integrals = []
    for i in 1:5
