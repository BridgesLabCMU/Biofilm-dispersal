...
function main()
    ...
    lines!(ax, 1:length(data_mean), random_mean, label="Random", color=:black, linestyle=:dash, colorrange=(1,7), linewidth=2)
    band!(ax, 1:length(random_mean), random_mean + random_std, random_mean - random_std, color=(:black, 0.1))
    writedlm("$(plots_folder)/rbmA_centroids_mean.csv", data_mean, ",")                                                                 
    writedlm("$(plots_folder)/rbmA_random_centroids_mean.csv", random_mean, ",")
    ax.xticks = xs
    ax.yticks = ys
